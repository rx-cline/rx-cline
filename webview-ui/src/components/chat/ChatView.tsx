...
const ChatView = ({ isHidden, showAnnouncement, hideAnnouncement, showHistoryView }: ChatViewProps) => {
	...
	*/
	...
	const placeholderText = useMemo(() => {
		const text = task ? "输入消息（@ 添加上下文）..." : "在此输入任务（@ 添加上下文）..."
		return text
	}, [task])
			...
			{task ? (
				...
				<div
					...
					<div style={{ padding: "0 20px", flexShrink: 0 }}>
						<h2>我能为您做些什么？</h2>
						<p>
							我可以一步步处理复杂的软件开发任务。借助能够让我创建和编辑文件、浏览复杂项目、使用浏览器以及执行终端命令的工具（在您授权后），我可以以超越代码完成或技术支持的方式协助您。甚至我还可以使用MCP创建新工具并扩展我的能力。
						</p>
					</div>
